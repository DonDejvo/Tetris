...
const int level_ticks[] = {800, 720, 640, 560, 480, 400, 320, 240, 160, 130, 100};
...
public:
    ...
    void translate_figure(int dx);
    void rotate_figure(RotDir rot_dir);
    void drop_figure();
    void move_down();
    ...
    inline void set_gameover()
    {
        gameover = true;
        top = std::max(score, top);
    }
    ...
    inline void set_config(const GameConfig &config) { this->config = config; };

private:
    GameConfig config;
    int width, height;
    std::function<int()> next_figure_cb = []()
